...
export default class AuthorDao {
    ...
    findByOrganizationId = (organizationId) => {
        const sql = 'SELECT * FROM author WHERE organizationId=? ORDER BY id ASC';
        const stmt = this.db.prepare(sql);
        return stmt.all(organizationId);
    }

    findAll = () => {
        const sql = 'SELECT author.id as author_id, author.organizationId as author_organizationId, author.uid as author_uid, author.name as author_name, organization.id as organization_id, organization.name as organization_name FROM author LEFT JOIN organization ON author.organizationId=organization.id ORDER BY author.id DESC';
        const stmt = this.db.prepare(sql);
