...
import { Box, Button, Dialog, DialogTitle, DialogContent, DialogActions, Input, InputLabel, TextField, Divider, Select, MenuItem } from '@mui/material';
...
export const AddAuthorsDialog = ({ status, setStatus, refresh }) => {
    ...
    const onSubmit = async () => {
        ...
        if (Object.keys(entity).length !== 0) {
            try {
                await RecordsApi.insert('authors', 'insert', entity, remark);
                onMessage({
                    type: 'success',
                    content: '已提交申请，待审核'
                });
    ...
    return (
        <Dialog fullscreen='true' fullWidth={true} maxWidth='sm' open={status} onClose={onClose}>
            ...
            <DialogContent>
                <Box sx={{ width: '50%', m: '1rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    <TextField
                        fullWidth
                        id="name-textfield"
                        label="名称"
